...
var distance = 7.0;
var zoomSpeed = 1.0;
private var distanceOffset = 0.0; //the change in the original distance value for zooming
var absoluteDistanceOffset = 3.0; //Max change from original distance in either the positive or negative direction

// the height we want the camera to be above the target
var height = 3.0;
private var heightOffset = 0.0;
var absoluteHeightOffset = 2.0;
...
//	DebugDrawStuff();
	...
	if (controller.IsJumping ())
	{
		...
		var newTargetHeight = targetCenter.y + (height + heightOffset);
		if (newTargetHeight < targetHeight || newTargetHeight - targetHeight > 5)
			targetHeight = targetCenter.y + (height + heightOffset);
	}
	...
	var currentRotX = cameraTransform.eulerAngles.x;
	var currentRotZ = cameraTransform.eulerAngles.z;
	var currentRotation = Quaternion.Euler (currentRotX, currentAngle, currentRotZ);
	
	// Set the position of the camera on the x-z plane to:
	// distance meters behind the target
	cameraTransform.position = targetHead;
	cameraTransform.position += currentRotation * Vector3.back * (distance + distanceOffset);
...
function LateUpdate () {
	//Get the value from the scroll wheel for zooming
	var zoomVal = (Input.GetAxis("Mouse ScrollWheel") * zoomSpeed) * -1.0;
	distanceOffset += zoomVal;
	heightOffset += zoomVal;
	if (distanceOffset < (-1.0 * absoluteDistanceOffset)) {
		distanceOffset = (-1.0 * absoluteDistanceOffset);
	}
	else if (distanceOffset > absoluteDistanceOffset) {
		distanceOffset = absoluteDistanceOffset;
	}
	if (heightOffset < (-1.0 * absoluteHeightOffset)) {
		heightOffset = (-1.0 * absoluteHeightOffset);
	}
	else if (heightOffset > absoluteHeightOffset) {
		heightOffset = absoluteHeightOffset;
	}
	if (Input.GetKeyDown("z")) {
		heightOffset = 0.0;
		distanceOffset = 0.0;
	}
	//Call the apply function to set the camera
